...
import { LinqDefaultError } from "../models/errors/linq-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | LinqDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | LinqDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new LinqDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new LinqDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
