 ...
 */
export function messagesGetMessageThread(
  client: LinqCore,
  messageId: string,
...
> {
  const input: operations.GetMessageThreadRequest = {
    messageId: messageId,
    cursor: cursor,
    limit: limit,
    order: order,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      z.parse(operations.GetMessageThreadRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/v3/messages/{messageId}/thread")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GetThreadResponse$inboundSchema),
