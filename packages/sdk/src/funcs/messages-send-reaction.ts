 ...
 */
export function messagesSendReaction(
  client: LinqCore,
  messageId: string,
...
> {
  const input: operations.SendReactionRequest = {
    messageId: messageId,
    body: body,
  };
  ...
  const parsed = safeParse(
    input,
    (value) => z.parse(operations.SendReactionRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/v3/messages/{messageId}/reactions")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.Reaction$inboundSchema),
