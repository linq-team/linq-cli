 ...
 */
export function messagesSendMessageToChat(
  client: LinqCore,
  chatId: string,
  ...
  Result<
    components.SendMessageResponse,
...
> {
  const input: operations.SendMessageToChatRequest = {
    chatId: chatId,
    body: body,
  };
  ...
  const parsed = safeParse(
    input,
    (value) =>
      z.parse(operations.SendMessageToChatRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/v3/chats/{chatId}/messages")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(202, components.SendMessageResponse$inboundSchema),
