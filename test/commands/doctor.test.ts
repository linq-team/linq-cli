...
// doctor now exits non-zero when checks fail. Tests here exercise both
// passing and failing scenarios but only assert on logged output, so
// swallow the ExitError that the failing paths raise.
async function runIgnoringExit(cmd: Doctor): Promise<void> {
  try {
    await cmd.run();
  } catch (e) {
    if (!(e instanceof Errors.ExitError)) throw e;
  }
}
...
describe('doctor', () => {
  ...
  it('masks token values in output', async () => {
    ...
    expect(output).toContain('my-secret-to...');
  });
});
